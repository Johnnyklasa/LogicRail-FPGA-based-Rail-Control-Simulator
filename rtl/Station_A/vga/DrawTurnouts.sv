...
module DrawTurnouts (
    ...
    // Redukcja obciążenia rejestrów (Fanout reduction)
    (* keep = "true" *) logic [11:0] hc_L1, vc_L1;
    (* keep = "true" *) logic [11:0] hc_L2, vc_L2;
    (* keep = "true" *) logic [11:0] hc_P1, vc_P1;
    (* keep = "true" *) logic [11:0] hc_P2, vc_P2;
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            hc_L1 <= '0; vc_L1 <= '0; hc_L2 <= '0; vc_L2 <= '0;
            hc_P1 <= '0; vc_P1 <= '0; hc_P2 <= '0; vc_P2 <= '0;
        end else begin
            ...
            hc_L1 <= vga_in.hcount; vc_L1 <= vga_in.vcount;
            hc_L2 <= vga_in.hcount; vc_L2 <= vga_in.vcount;
            hc_P1 <= vga_in.hcount; vc_P1 <= vga_in.vcount;
            hc_P2 <= vga_in.hcount; vc_P2 <= vga_in.vcount;
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // Wykorzystanie sklonowanych rejestrów do obliczeń w potoku
            d_L1_reg <= en_L1 ? IsLine(hc_L1, vc_L1, 80,  L1_sy, 200, L1_dy, TOR_HEIGHT) : 1'b0;
            d_L2_reg <= en_L2 ? IsLine(hc_L2, vc_L2, 80,  L2_sy, 200, L2_dy, TOR_HEIGHT) : 1'b0;
            d_P1_reg <= en_P1 ? IsLine(hc_P1, vc_P1, 520, P1_sy, 200, P1_dy, TOR_HEIGHT) : 1'b0;
            d_P2_reg <= en_P2 ? IsLine(hc_P2, vc_P2, 520, P2_sy, 200, P2_dy, TOR_HEIGHT) : 1'b0;
